    ...
    <!-- ################################################################################################ -->
          ...
          <div>
              <div class="hoc container clear">
                <!-- ################################################################################################ -->
                <table>
                  <tr style="font-size: 40px;border-bottom: 1px solid white;"><th>Your Uploads</th><th>Interested Users</th></tr>

                  <?php
                    $raw_results = mysqli_query($con,"SELECT * FROM `book_details` where uid='".$uid."'");
                      //display
                      if(mysqli_num_rows($raw_results) > 0){ // if one or more rows are returned do following
                                        ...
                                        while($results = mysqli_fetch_array($raw_results)){
                                          ...
                                          echo '<tr>
                                                <td class="heading">'.$results['name'].'</td><td>';
                                            $query = mysqli_query($con,"SELECT * FROM `confirm` where seller_id='".$uid."' and bid='".$results['bid']."'");
                                            if(mysqli_num_rows($query) > 0){

                                                while ($rows = mysqli_fetch_array($query)) {

                                                  $q = mysqli_query($con,"SELECT * FROM `credentials` where id='".$rows['buyer_id']."'");
                                                  if(mysqli_num_rows($q) > 0){
                                                    while ($q1 = mysqli_fetch_array($q)) {

                                                      echo '<p><a style="color:white;" href="profile.php?buyer='.$q1['name'].' & bid='.$results['bid'].'">'.$q1['name'].'</a></p>';
                                                    }
